...
typedef struct {
    ...
    XMin_max_shmem_uio_map maps[ MAX_UIO_MAPS ];
} XMin_max_shmem_uio_info;

/***************** Variable Definitions **************************************/
static XMin_max_shmem_uio_info uio_info;
...
static int uio_info_read_name(XMin_max_shmem_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMin_max_shmem_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMin_max_shmem_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMin_max_shmem_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMin_max_shmem_Initialize(XMin_max_shmem *InstancePtr, const char* InstanceName) {
	XMin_max_shmem_uio_info *InfoPtr = &uio_info;
