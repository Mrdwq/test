...
VSI_LOOP_0: for (int idx = 0 ; idx < (NROWS*NCOLS) ; idx++ ) {
...
VSI_LOOP_1: for (int idy = 0 ; idy < NROWS ; idy++ ) {
VSI_LOOP_2: for (int idx = 0 ; idx < NCOLS ; idx++) {
			#pragma HLS PIPELINE II=1
...
VSI_LOOP_3: for (int idy = 1 ; idy < NROWS-1; idy++) {
VSI_LOOP_4: for (int idx = 1; idx < (NCOLS-1); idx++) {
#pragma HLS PIPELINE II=1
			uint32_t data = 0;
...
static void fast_corner (hls::stream<uint8_t> &ins, hls::stream<uint8_t> &outs)
{
	...
	xf::Mat<XF_8UC1,NROWS,NCOLS,XF_NPPC1> cam_mat_i(NROWS,NCOLS);
	// read input
VSI_LOOP_5: for (int idx = 0 ; idx < (NROWS*NCOLS) ; idx++ ) {
#pragma HLS PIPELINE II=1
		uint8_t td = ins.read();
		cam_mat.data[idx] = (uint8_t) td;
	}
	...
	int  n_points = 0;
VSI_LOOP_6: for (int j = 0; j < cam_mat_i.rows; j++) {
VSI_LOOP_7: for (int i = 0; i < (cam_mat_i.cols>>XF_BITSHIFT(XF_NPPC1)); i++) {
#pragma HLS PIPELINE II=1
	...
	// send output
VSI_LOOP_8: for (int idx = 0 ; idx < (NROWS*NCOLS) ; idx++ ) {
#pragma HLS PIPELINE II=1
...
void MeanShift (hls::stream<uint32_t> &ins, hls::stream<uint16_t> &track_in, hls::stream <uint16_t> &outs)
{
	...
	if (!frame_status) {
VSI_LOOP_9: for (int i = 0 ; i < XF_MAX_OBJECTS; i++) {
			dx[i] = 0;
			dy[i] = 0;
			h_x[i] = 0;
			h_y[i] = 0;
...
VSI_LOOP_10: for (int i = 0 ; i < (XF_HEIGHT*XF_WIDTH) ; i++) {
#pragma HLS PIPELINE II=1
...
VSI_LOOP_11: for (int k = 0; k < XF_MAX_OBJECTS; k++) {
		// out of range
...
VSI_LOOP_12: for (int k = 0; k < XF_MAX_OBJECTS; k++) {
#pragma HLS PIPELINE II=1
		outs.write(track[k]);
		outs.write(tlx[k]);
		outs.write(tly[k]);
...
VSI_LOOP_13: for (int i = 0 ; i < XF_HEIGHT ; i++) {
VSI_LOOP_14: for (int j = 0 ; j < XF_WIDTH ; j++) {
#pragma HLS PIPELINE II=1
			outa[(i*XF_WIDTH)+j] = ins.read();
		}
	}
VSI_LOOP_15: for (int idy = ED_SIZE ; idy < (XF_HEIGHT-ED_SIZE); idy+=ED_SIZE) {
VSI_LOOP_16: for (int idx = ED_SIZE; idx < (XF_WIDTH-ED_SIZE); idx+=ED_SIZE) {
			uint32_t data = 0;
			uint32_t max_val = 0;
VSI_LOOP_17: for (int i = -ED_SIZE/2 ; i < ED_SIZE/2 ; i ++)
VSI_LOOP_18: for (int j = -ED_SIZE/2 ; j < ED_SIZE/2; j++)
#pragma HLS PIPELINE II=1
					if (outa[((idy+i)*XF_WIDTH)+idx+j] > max_val) max_val = outa[((idy+i)*XF_WIDTH)+idx+j];
VSI_LOOP_19: for (int i = -ED_SIZE/2 ; i < ED_SIZE/2 ; i ++)
VSI_LOOP_20: for (int j = -ED_SIZE/2 ; j < ED_SIZE/2; j++)
#pragma HLS PIPELINE II=1
...
void vsi_erode (hls::stream<uint8_t> &ins, uint32_t outx[XF_HEIGHT*XF_WIDTH/4]) {
	uint8_t *outa = (uint8_t *)&outx[0];
VSI_LOOP_21: for (int i = 0 ; i < (XF_HEIGHT) ; i++)
VSI_LOOP_22: for (int j = 0 ; j < XF_WIDTH ; j++)
#pragma HLS PIPELINE II=1
			outa[(i*XF_WIDTH)+j] = ins.read();

VSI_LOOP_23: for (int idy = ED_SIZE ; idy < (XF_HEIGHT-ED_SIZE); idy+=ED_SIZE) {
VSI_LOOP_24: for (int idx = ED_SIZE; idx < (XF_WIDTH-ED_SIZE); idx+=ED_SIZE) {
			uint32_t data = 0;
			uint32_t min_val = -1;
VSI_LOOP_25: for (int i = -ED_SIZE/2 ; i < ED_SIZE/2 ; i ++)
VSI_LOOP_26: for (int j = -ED_SIZE/2 ; j < ED_SIZE/2; j++)
#pragma HLS PIPELINE II=1
					if (outa[((idy+i)*XF_WIDTH)+idx+j] < min_val) min_val = outa[((idy+i)*XF_WIDTH)+idx+j];

VSI_LOOP_27: for (int i = -ED_SIZE/2 ; i < ED_SIZE/2 ; i ++)
VSI_LOOP_28: for (int j = -ED_SIZE/2 ; j < ED_SIZE/2; j++)
#pragma HLS PIPELINE II=1
...
VSI_LOOP_30: for (int i = 0 , j =0; i < FC_ROWS*FC_COLS/4;i++,j+=4) {
#pragma HLS PIPELINE II=1
	...
	int  n_points = 0;
VSI_LOOP_31: for (int j = 0; j < cam_mat.rows; j++) {
VSI_LOOP_32: for (int i = 0; i < (cam_mat.cols>>XF_BITSHIFT(XF_NPPC1)); i++) {
#pragma HLS PIPELINE II=1
...
VSI_LOOP_33: for (int i = 0 , j =0; i < FC_ROWS*FC_COLS/4;i++,j+=4) {
#pragma HLS PIPELINE II=1
		uint32_t val = io_frame[i];
		cam_mat_i.data[j]   = (val >> 24) & 0xff;
		cam_mat_i.data[j+1] = (val >> 16) & 0xff;
	...
	// do median_blur and minmax in the same loop
VSI_LOOP_34: for (int idy = 1 ; idy < FC_ROWS-2; idy++) {
VSI_LOOP_35: for (int idx = 1; idx < (FC_COLS-2); idx++) {
#pragma HLS PIPELINE II=1
			uint32_t data = 0;
...
VSI_LOOP_36: for (int deg = 0 ; deg <= 360; deg += DEG_INC) {
#pragma HLS PIPELINE
